...
class CosineAnnealingLR_with_Restart(_LRScheduler):
    ...
    teh restarts part implemented from http://jsideas.net/python/2018/03/14/snapshot_ensemble.html.
    ...
    def get_lr(self):
        #print("base_lrs",self.base_lrs)
        #print("self.eta_min + (base_lr - self.eta_min) = {:5f}".format(self.eta_min + (self.base_lrs[0] - self.eta_min)))
        #print("(1 + math.cos(math.pi * self.current_epoch / self.Te)) / 2 = {:5f}".
        #      format((1 + math.cos(math.pi * self.current_epoch / self.Te)) / 2))
        new_lrs = [self.eta_min + (base_lr - self.eta_min) *
                (1 + math.cos(math.pi * self.current_epoch / self.Te)) / 2

                for base_lr in self.base_lrs]
        #print("get_lr :",new_lrs)
    ...
    def step(self, epoch=None):
        ...
        if self.current_epoch == self.Te:
            ...
            #print("next goal", self.Te)
            self.T_max = self.T_max + self.Te
            #print("T_max", self.T_max)
            
    def state_dict(self):
        return {key: value for key, value in self.__dict__.items() if key not in {'optimizer', 'is_better', 'model'}} 
    
    def load_state_dict(self, state_dict, model):
        self.T_max = state_dict['T_max']
        self.T_mult = state_dict['T_mult']
        self.Te = state_dict['Te']
        self.eta_min = state_dict['eta_min']
        self.current_epoch = state_dict['current_epoch']  
        self.last_epoch = state_dict['last_epoch']
        self.model = model
        self.out_dir = state_dict['out_dir']
        self.take_snapshot = state_dict['take_snapshot']
...
class CyclicLR(_LRScheduler):
    ...
    def __init__(self, optimizer, base_lr=1e-3, max_lr=6e-3,
        ...
        super(CyclicLR, self).__init__(optimizer, last_batch_iteration)
